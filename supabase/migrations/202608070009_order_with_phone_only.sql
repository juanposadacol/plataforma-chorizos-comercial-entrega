-- El celular identifica al comprador: se elimina el requisito de OTP para pedir.
--
-- PROBLEMA (reportado desde la tienda en producción)
-- `create_order` rechazaba con `CUSTOMER_AUTH_REQUIRED` cualquier pedido de
-- invitado cuyo celular YA estuviera registrado, y mandaba a iniciar sesión con
-- un código SMS. Como el proyecto no tiene proveedor de SMS configurado, ese
-- acceso responde «Unsupported phone provider»: el cliente que vuelve a comprar
-- quedaba encerrado sin ninguna salida —ni pedía, ni podía autenticarse—.
-- En la práctica solo podían comprar los celulares que nunca habían pedido.
--
-- DECISIÓN DEL NEGOCIO
-- El comprador no inicia sesión. Escribe su celular y con eso se le reconoce,
-- se le autocompleta la entrega y se le aplica su lista de precios. Es la misma
-- decisión ya tomada en 202608070004 para la búsqueda por celular
-- («con una base de pocas decenas de clientes, exigir sesión solo estorbaba al
-- cliente real que vuelve a comprar»), extendida ahora a la creación del pedido.
--
-- LO QUE ESTO IMPLICA, EXPLÍCITAMENTE
-- Quien escriba el celular de un cliente registrado puede crear un pedido a su
-- nombre y ver los precios de su lista. El negocio confirma cada pedido antes
-- de despachar, así que el control real queda en esa confirmación. NO se expone
-- nada más: documento, correo, saldo, cupo e historial siguen sin salir del
-- panel, y `get_my_orders`/`customer_self` siguen exigiendo sesión.
--
-- Se conserva la protección contra suplantación ENTRE SESIONES: un usuario
-- autenticado no puede pedir a nombre de un cliente vinculado a otra cuenta.
-- Se conservan también el rechazo de clientes inactivos o bloqueados, el
-- control de precios en el servidor y el resto de la función sin cambios.
